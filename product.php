...
$sql = /** @lang text */
    ...
    from product as p
	join product_image_main as pim join image as i on p.id = pim.product_id and pim.image_id = i.id
    join section_product_main as spm join section as s on p.id = spm.product_id and spm.section_id = s.id
    where p.id = :id;";
...
$sql = /** @lang text */
	...
	join image as i on pia.image_id = i.id
...
$sql = /** @lang text */
	...
	join section as s on spa.section_id = s.id
...
foreach ($add_sections as $row) {
    $add_sections_html .= /** @lang text */
        "<a href='index.php?sect_id=$row[product_additional_section_id]'>$row[product_additional_section]</a>";
}
...
if ($referer['host'] == $_SERVER['SERVER_NAME']
    && $referer['path'] == '/index.php'
    && $referer['query'] != '')
{
    parse_str($referer['query'], $queries);
    foreach ($add_sections as $row)
    {
        if ($row['product_additional_section_id'] == (int) $queries['sect_id'])
        {
...
<div class="product">
    ...
    <div class="product__description">
        <label><a href="index.php?sect_id=<?= $back_section_id ?>">Назад</a></label>
        <h1 class="product_title"><?= mb_strtoupper($product_info['product_title']) ?></h1>
        <div class="product__categories">
            <a href="index.php?sect_id=<?= $product_info['product_main_section_id'] ?>"><?= $product_info['product_main_section'] ?></a>
            <?= $add_sections_html ?>
        </div>
